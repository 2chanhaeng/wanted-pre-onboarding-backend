...
class NoticeSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        if {'id', 'date', 'application', 'other_notice'} & set(self.initial_data):
            raise serializers.ValidationError("Notice date and application are not allowed")
        # rawdata에서 제외할 데이터
        not_from_rawdata = {'name', 'company', 'csrfmiddlewaretoken'}
        # rawdata에서 제외한 데이터를 data에 추가 후 반환
        return data | {k:v for k, v in self.initial_data.items() if k not in not_from_rawdata}
    ...
    def to_representation(self, instance:Notice):
        ...
        return {
            ...
            "other_notice": [notice.id for notice in instance.company.notice_set.exclude(id=instance.id)],
            "date": instance.date,
...
class CompanySerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance:Company):
        ...
        return {
            ...
            "notice": [NoticeSerializer.to_representation_in_company(notice) for notice in instance.notice_set.all()],
            } | (json.loads(instance.info) if instance.info else {})
...
class UserSerializer(serializers.ModelSerializer):
    ...
    def to_representation(self, instance:Company):
        ...
        return {
            ...
            "application": self.fields['application'].to_representation(instance.application_set.all()),
            } | (json.loads(instance.info) if instance.info else {})
